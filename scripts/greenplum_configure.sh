...
} >> "${home_dir}/gp_data/gpseg-1/pg_hba.conf"
...
while ! PGPORT=${PORT} GPHOME="${gp_home}" COORDINATOR_DATA_DIRECTORY="${home_dir}/gp_data/gpseg-1"  "${gp_home}/bin/gpconfig" -s gp_vmem_protect_limit; do
  ./scripts/greenplum_restart.sh
done
