...
numactl -N "${node_id}" ./scripts/db_comparison_runner.py monetdb --clients "${num_clients}" --cores "${num_cpu}" -m "${node_id}"
numactl -N "${node_id}" ./scripts/db_comparison_runner.py monetdb --clients "${num_clients}" --cores "${num_cpu}" -m "${node_id}" --skip_data_loading

num_segments=$([ "$num_cpu" -le 14 ] && echo "$num_cpu" || echo "14")
./python/greenplum_configure.py -p 7777 -n "$num_segments"
PORT=7777 ./scripts/greenplum_init
numactl -N "${node_id}" ./scripts/db_comparison_runner.py greenplum --clients "${num_clients}" --cores "${num_cpu}" -m "${node_id}"
numactl -N "${node_id}" ./scripts/db_comparison_runner.py greenplum --clients "${num_clients}" --cores "${num_cpu}" -m "${node_id}" --skip_data_loading
